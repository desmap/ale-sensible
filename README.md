...
**The first time, I installed ALE, I uninstalled it right away.** ALE is mighty but its power is hidden behind subpar defaults and a unintuitive, stuttery, laggy UI. It's just a few settings to change but it took me a day crawling through helpfiles and Github issues to find the right ones and then, cluttering my .vimrc/init.vim with odd lines for ALE. It's ok to spend time configuring your editor but your linter?? Don't waste time like I did and use sane defaults from the first second. Introducing ALE SENSIBLE:
...
If you have already a line with `'w0rp/ale'`, remove this line (because we have to make sure that new overriding configs load before ALE loads, some ALE oddity, hence the order in the install lines above). Also remove all your autocomplete plugins, ALE brings a great one along which ALE SENSIBLE finally turns on and maps to `<Tab>` in Insert Mode. If you want to use another autocomplete just keep the `<Tab>` remapping to that other one in your config, it will overwrite ALE's one. Why it might still make sense to use ALE's autocomplete: It's built in, uses the same core and is well maintained, why install and config something twice? Supertab doesn't seem to be maintained anymore, YouCompleteMe and Deoplete need further dependencies, are much heavier and like also some config work.
